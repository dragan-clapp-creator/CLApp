...
public class ClpTokenLogicVisitor {

  private boolean istrue;
  private Resources res;
  private TokenHandler activator;

  public ClpTokenLogicVisitor(Resources r, TokenHandler tokenHandler) {
    super();
    res = r;
    activator = tokenHandler;
  }
  ...
  public void visitCellFunction(CellFunction x, boolean isNOT) {
    ...
    switch (x.getFunction()) {
      case ACTIVE:
        istrue = activator.isActiveInQueue(name) || EventHandler.getInstance().isUp(name) == Boolean.TRUE;
        break;
      case INACTIVE:
        istrue = activator.isInactiveInQueue(name) || EventHandler.getInstance().isDown(name) == Boolean.TRUE;
        break;
    }
    if (isNOT) {
      istrue = !istrue;
  ...
  public void visitLexpression(Lexpression x, boolean isNOT) {
    istrue = activator.isTermTrue(res, x.getLogicalExp().getLogicalTerm());
    if (!istrue) {
      for (int i=0; !istrue && i<x.getLogicalExp().getLogicalTerms().size(); i++) {
        LogicalTerm term = x.getLogicalExp().getLogicalTerms().get(i);
        istrue = activator.isTermTrue(res, term);
      }
    }
    if (isNOT) {
      istrue = !istrue;
